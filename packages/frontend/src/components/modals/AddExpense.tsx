...
import { ChevronDown, Lock, Scan } from "lucide-react";
  ...
  return (
    <form className="space-y-4 px-5 pb-5 sm:px-6 sm:pb-6" onSubmit={handleSubmit}>
      <div className="space-y-2">
        <label className="ml-1 block text-[10px] font-bold uppercase tracking-widest text-primary">Expense Name</label>
        <input
          required
          className="w-full rounded-xl border-none bg-surface-container px-4 py-3 text-on-surface placeholder:text-on-surface-variant/40 transition-all focus:bg-surface-bright focus:ring-1 focus:ring-primary"
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <div className="absolute left-4 top-1/2 -translate-y-1/2 text-base font-bold text-primary">EUR</div>
          <input
            required
            className="font-headline w-full rounded-xl border-none bg-surface-container py-4 pl-16 pr-4 text-2xl font-black text-on-surface placeholder:text-on-surface-variant/20 transition-all focus:bg-surface-bright focus:ring-1 focus:ring-primary sm:text-3xl"
            inputMode="decimal"
            pattern="[0-9]*[.,]?[0-9]*"
            placeholder="0.00"
            type="text"
      ...
      <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center sm:justify-between">
        <button
          type="button"
          onClick={onScan}
          className="flex items-center justify-center gap-2 rounded-full bg-surface-container-highest px-5 py-3 text-sm font-bold text-primary transition-all hover:brightness-110 active:scale-95"
        >
      ...
      <div className="-mx-5 -mb-5 mt-5 flex items-center gap-2 bg-surface-container-lowest px-5 py-3 sm:-mx-6 sm:-mb-6 sm:px-6">
