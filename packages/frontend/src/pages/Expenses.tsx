...
export default function Expenses({ onAddExpense, onEditExpense, onDeleteExpense, refetchRef }: ExpensesProps) {
  ...
  const fetchExpenses = useCallback(() => {
    ...
    const input =
      selectedKategorie === "Alle" ? {} : { kategorie: selectedKategorie as Exclude<KategorieFilter, "Alle"> };

    trpcClient.ausgaben.list
      .query(input)
      .then((data) => setExpenses(data as Expense[]))
      .catch((err: unknown) => setError(err instanceof Error ? err.message : "Fehler beim Laden der Ausgaben"))
  ...
  return (
    <div className="px-4 pb-8 pt-4 sm:px-6 lg:px-12 lg:pb-12">
      ...
      <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between lg:mb-12">
        <div>
          <h2 className="font-headline text-3xl font-extrabold tracking-tight text-on-surface sm:text-4xl">Ausgaben</h2>
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-primary-container to-primary px-5 py-2.5 font-bold text-on-primary transition-all emerald-glow hover:brightness-110 active:scale-95 sm:px-6"
        >
      ...
      <section className="mb-8 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <div className="grid grid-cols-2 gap-4 sm:flex sm:gap-12">
          <div className="space-y-1">
            <p className="text-xs font-medium text-on-surface-variant uppercase tracking-widest">Anzahl Ausgaben</p>
            <p className="text-3xl font-bold font-headline">{sorted.length}</p>
          </div>
          <div className="space-y-1">
            <p className="text-xs font-medium text-on-surface-variant uppercase tracking-widest">Gesamtbetrag</p>
            <p className="text-3xl font-bold font-headline text-primary">€{totalBetrag.toFixed(2)}</p>
          </div>
        </div>
      ...
      {loading && (
        <div className="flex items-center justify-center py-24">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-on-surface-variant text-sm font-label tracking-widest uppercase">Lade Ausgaben…</p>
          </div>
        </div>
      )}
      ...
      {!loading && !error && (
        <>
          <div className="overflow-hidden rounded-2xl border border-outline-variant/10 bg-surface-container-low shadow-2xl">
              ...
              <table className="min-w-[760px] w-full border-collapse text-left">
                <thead>
                  <tr className="bg-surface-container/50 border-b border-outline-variant/10">
                    <th className="px-6 py-5 text-xs font-semibold uppercase tracking-widest text-on-surface-variant">
                      Titel
                    </th>
                    <th className="px-6 py-5 text-xs font-semibold uppercase tracking-widest text-on-surface-variant">
                      Kategorie
                    ...
                    <th className="px-6 py-5 text-xs font-semibold uppercase tracking-widest text-on-surface-variant text-right">
                      Betrag
                    </th>
                    <th className="px-6 py-5 text-xs font-semibold uppercase tracking-widest text-on-surface-variant text-center">
                      Aktionen
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-outline-variant/5">
                  {paginated.length === 0 ? (
                    ...
                    paginated.map((expense) => {
                      ...
                      return (
                        <tr key={expense.id} className="hover:bg-surface-container transition-colors group">
                          ...
                          <td className="px-6 py-4 text-sm text-on-surface-variant">{formatDate(expense.datum)}</td>
            ...
            {/* Pagination footer */}
            <div className="flex flex-col gap-3 border-t border-outline-variant/10 bg-surface-container-low p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-3">
            <div className="flex items-start gap-4 rounded-2xl border border-primary/10 bg-primary-container/5 p-5 md:col-span-2 sm:gap-5 sm:p-6">
              ...
              <div>
                ...
                <p className="text-sm text-on-surface-variant leading-relaxed">
                  Navigiere zu „AI Tipps&quot;, um personalisierte Spartipps basierend auf deinen aktuellen Ausgaben zu
                  erhalten. Alle Berechnungen erfolgen lokal.
                </p>
              </div>
            </div>
